...
function setOriginalImage() {
  ...
  originalImage.setAttribute(
    'src',
    `./images/slide_puzzle/${selectedImage}.png`
  );
}
...
function renderTiles(arr) {
  ...
  arr.forEach((tile, index) => {
    ...
    div.style.backgroundImage = `url(./images/slide_puzzle/${selectedImage}/${level}/tiles${tile}.png)`;
...
function start() {
  setOriginalImage();
  count = 0;
  tilesArray = generateShuffledArray(orderArray);
